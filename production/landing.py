...
# ── GPU fleet (3× g6e.2xlarge, EC2 tag Fleet=wan, on-demand) ────────────────
# Boxes stop themselves after 15 idle minutes and get a new public IP on every
# start, so discovery + boot live in video_gen (ec2:DescribeInstances /
# ec2:StartInstances by tag — no instance IDs or IPs pinned here).
def _ensure_fleet():
    """Warm the on-demand Wan fleet: start stopped boxes and wait for a worker to answer.
    Best-effort — video_gen.generate() re-ensures per job, so a failure here only costs
    the head start."""
    try:
        video_gen.ensure_workers()
    except Exception as e:
        print(f"[fleet] warm failed: {e}", flush=True)
